#!/bin/bash
# run_mirage_tokyo.sh  --  Tokyo only
# Usage:  bash scripts/run_mirage_tokyo.sh
set -e
...
echo "[3/6] Training..."
python train.py --config "src/config/config_mirage_tokyo.yaml"
...
python generate.py \
    --config "src/config/config_mirage_tokyo.yaml" \
