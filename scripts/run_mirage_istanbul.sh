#!/bin/bash
# run_mirage_istanbul.sh  --  Istanbul only
# Usage:  bash scripts/run_mirage_istanbul.sh
set -e
...
echo "[3/6] Training..."
python train.py --config "src/config/config_mirage_istanbul.yaml"
...
python generate.py \
    --config "src/config/config_mirage_istanbul.yaml" \
